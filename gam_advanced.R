...
equation <- Load ~ s(as.numeric(Date), k = 3, bs = "cr") +
  s(toy, k = 30, bs = "cr") + s(Temp, k = 10, bs = "cr") +
  s(Load.1, bs = "cr") + s(Load.7, bs = "cr") +
  s(Temp_s99, k = 10, bs = "cr") + as.factor(WeekDays) + BH +
  te(Temp_s99, Temp, bs = c("cr","cr"), k = c(10, 10))
...
equation <- Load ~ s(as.numeric(Date), k = 3, bs = "cr") +
  s(toy, k = 30, bs = "cc") + ti(Temp, k = 10, bs = "cr") + 
  s(Load.1, bs = "cr")+ s(Load.7, bs = "cr") +
  ti(Temp_s99, k = 10, bs = 'cr') + as.factor(WeekDays) + BH +
...
equation2 <- Load ~ ti(Load.1, bs = "cr") + ti(Load.7, bs = "cr") +
  ti(Load.1, Load.7)
...
equation_cs <- Load ~ s(as.numeric(Date), k = 3, bs = "cr") +
  s(toy, k = 30, bs = "cc") + s(Temp, k = 10, bs = "cr") +
  s(Load.1, bs = "cr", by = as.factor(WeekDays)) + s(Load.7, bs = "cr") +
  s(Temp_s99, k = 10, bs = "cr") + as.factor(WeekDays) + BH  + BH_before +
  BH_after + s(Temp_s95) + ti(Nebulosity_weighted) + ti(Wind_weighted) +
  ti(Wind_weighted, Temp) + s(Var1, bs = "cs")
# equation_cs <- Load~s(as.numeric(Date),k=3, bs='cs') +
  #s(toy,k=30, bs='cr') + s(Temp,k=10, bs='cr') +
  #s(Load.1, bs='cs',  by= as.factor(WeekDays))+ s(Load.7, bs='cr') +
  #s(Temp_s99,k=10, bs='cs') + as.factor(WeekDays) + BH  + BH_before +
  #BH_after +s(Temp_s95, bs='cs') +ti(Nebulosity_weighted, bs='ts') +
  #ti(Wind_weighted, bs='ts')+ti(Wind_weighted, Temp, bs='ts') + s(Var1, bs='cs')
...
o <- order(Data0$toy)
plot(Data0$toy[o], terms[o, sel.column], type = 'l',
     ylim = range(terms[o, sel.column], terms_cs[o, sel.column]))
...
plot(Data0$Wind_weighted[o], terms[o, sel.column], type = 'l',
     ylim = range(terms[o, sel.column], terms_cs[o, sel.column]))
...
o <- order(Data0$Wind_weighted)
plot(Data0$Var1[o], terms[o, sel.column], type = 'l',
     ylim = range(terms[o, sel.column], terms_cs[o, sel.column]))
...
ssm_em <- viking::select_Kalman_variances(ssm, X[sel_a,], y[sel_a], method = 'em', n_iter = 10^3,
                                          Q_init = diag(d), verbose = 100, mode_diag = T)
...
plot(ssm_em, pause = FALSE, window_size = 14, date = Data0$Date, sel = sel_b)
...
r <- range(cumsum(Data0$Load[sel_b] - gam9.kalman.Dyn),
           cumsum(Data0$Load[sel_b]- gam9.forecast))
plot(Data0$Date[sel_b], cumsum(Data0$Load[sel_b] - gam9.forecast),
     type = 'l', ylim = r)
lines(Data0$Date[sel_b], cumsum(Data0$Load[sel_b] - gam9.kalman.static),
      col = 'blue')
lines(Data0$Date[sel_b], cumsum(Data0$Load[sel_b] - gam9.kalman.Dyn),
      col = 'green')
lines(Data0$Date[sel_b], cumsum(Data0$Load[sel_b] - gam9.kalman.Dyn.em),
      col = 'purple')
...
lines(Data0$Date[sel_b], cumsum(Data0$Load[sel_b] - gam9.kalman.Dyn.em),
      col = 'purple')
lines(Data0$Date[sel_b], cumsum(Data0$Load[sel_b] - gam9.kalman.Dyn2),
      col = 'orange')
...
#discrete=TRUE, qu=0.05)
#X <- cbind(predict(gqgam05, newdata=Data0, type='terms'),
...
pinball_loss(y = Data0$Load[sel_b], gqgam.kalman.Dyn+quant,
             quant = 0.95, output.vect = FALSE)


gqgam05 <- qgam(list(equation, equation_var), data = Data0[sel_a, ],
                discrete = TRUE, qu = 0.5)
