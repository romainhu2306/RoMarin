...
plot(test$Date, cumsum(final_qgam_res),
     type = "l", col = "red", main = "Cumulative sum of the residuals",
     ylab = "Cumulative sum of the residuals", xlab = "Date", lwd = 2)
lines(cumsum(final_xgb_res), type = "l", col = "blue",
      lwd = 2)
legend("topleft", c("qgam alone", "qgam + xgboost"), col = c("red", "blue"),
       lty = 1)
...
```{r Online learning with Kalman filtering}
...
boosted_pred <- xgb_pred + qgam_pred
...
ssm_em_pred <- predict(ssm_em, input, target, type = "model",
                       compute_smooth = TRUE)
final_kf_pred <- ssm_em_pred$pred_mean[-sel_a]
print(paste("Pinball loss:", pinball_loss(test$Net_demand,
                                          final_kf_pred, quant = .8,
                                          output.vect = FALSE)))
print(paste("RMSE:", rmse(test$Net_demand, final_kf_pred)))
